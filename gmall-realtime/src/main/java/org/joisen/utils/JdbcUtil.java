...
public class JdbcUtil {
    ...
    public static <T> List<T> queryList(Connection connection, String sql, Class<T> clz, boolean underScoreToCamel) throws SQLException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        while (resultSet.next()) { // 行遍历
            // 创建T对象
            T t = clz.newInstance();
            for (int i = 0; i < columnCount; i++) { // 列遍历, 并给T对象赋值
                // 获取列名与列值
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<JSONObject> jsonObjects = queryList(connection, "select * from GMALL0105_REALTIME.DIM_BASE_TRADEMARK", JSONObject.class, false);
